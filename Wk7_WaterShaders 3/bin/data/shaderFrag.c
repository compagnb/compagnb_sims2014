...
//uniform float myArray[N]; //passing an array

float cnoise (vec3 P); //classic perlin noise function declaration
...
void main(){
    ...
    //Shift the position using perilin noise
    vec2 shift;
    shift.x = cnoise(vec3(pos * 0.02, time * 0.5 + 17.0)) * 30;
    shift.y = cnoise(vec3(pos * 0.02, time * 0.5 + 17.0)) * 30;
    pos += shift;
